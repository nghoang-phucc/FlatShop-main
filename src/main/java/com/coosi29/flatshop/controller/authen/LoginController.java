...
//	  @Autowired private AuthenticationManager authenticationManager;
	...
	@GetMapping(value = "/homes") // chuyen den home khi dang nhap thanh cong
	...
	public String login (HttpSession session, @RequestParam(name = "account") String account,
		...
		try {
			...
			if (userDTO.getEmail() != null) {
				...
				Long role = userDTO.getRoleDTO().getRoleId();
				if ( role == 1) {
					return "redirect:/admin/home";
				}
				return "redirect:/client/home";
			} else {
				request.setAttribute("err", "Please check email or password not found?");
				return "authen/login";
			}
		} catch (Exception e) {
